...
Each project folder under ``dags/`` is its own Python package (an
``__init__.py`` plus a ``common/`` subpackage), so adding a second project
alongside ``demo`` never risks a helper-name collision. ``dags/`` is on
``sys.path`` and this project's package is ``demo``, so ``from
demo.common.greetings import ...`` resolves from any pipeline in the project. If this import ever breaks, every importer shows up
