"""Shared helpers for the ``project1`` project's pipelines.
...
another's helpers, so ``project1`` owns this copy outright and may change it
without touching ``demo`` or ``project2``. The parent of the project folders
is on ``sys.path`` and this project's package is ``project1``, so
``from project1.common.greetings import ...`` resolves from any pipeline in
the project.
